...
本项目是一个具有“魔术效果”的网页计算器，外观复刻 iOS26 的计算器，核心逻辑为：初始阶段功能正常，首次按等号记录结果，首次按等号后的首次清屏后，基于系统时间生成7位或8位数并暗中计算差值，随后通过点击网页任意位置逐位显示差值，只有显示完毕后才可点击清屏键清屏，清屏后计算器恢复正常功能。

## 二、外观设计

完全复刻 iOS26 计算器的UI布局、比例、颜色、字体、动画，包括但不限于：

1. **显示区**：位于顶部，用于显示输入的计算式、输出的计算结果、差值的逐位显示内容。
2. **按键区**：包含数字键（0-9）、运算符号键（+、-、×、÷等）、等号键（=）、回退键（⌫）、清屏键（AC/C，需明确清屏键的标识）。
...
- **功能表现**：
  1. 计算器功能基本正常，支持数字输入、运算符号选择、连续运算、回退、清屏等正常功能。
  2. 显示区实时响应用户操作，显示输入的数字、运算式等。
  3. 显示逻辑和动画复刻 iOS26 计算器。
- **状态流转触发**：用户点击“等号键”。
...
- **功能表现**：
  ...
  3. 当变量 `R` 存在时，用户点击网页任意位置（清屏键除外），显示区都不会有任何变化。
- **状态流转触发**：用户点击“清屏键”。
...
**仅使用 `HTML+CSS+JavaScript`，不使用 `Node.js`、`TypeScript`、`React` 等。**
